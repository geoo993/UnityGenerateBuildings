...
//		}
		...
		if (xSize > 20  || zSize > 20) {

			print (xSize);

	
			int xCount = 1;
			while (xSize/xCount > 20)
			{
...
//
//				Vector3 zp = new Vector3 (center.transform.position.x, center.transform.position.y, zP);
//
...
//			}
			...
			for (int s = 0; s < xCount; s++) {
				...
				for (int z = 0; z < zCount; z++) {
					...
					Vector3 zp = new Vector3 (xP, center.transform.position.y + 2f, zP);
		...
		if (xSize > 10 && zSize > 10){
			...
			print(" x yess:  "+ xSize + "   z yess:  "+ zSize);
			xSize -= 10;
			zSize -= 10;
		}
...
//
	...
	void OnGUI() {
		...
		GUI.Label(new Rect(610, 10, 200, 20), ("Press Space ").ToString());
