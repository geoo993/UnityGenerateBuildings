Shader ".ShaderExample/TextureSplatting"
{
...
//		[NoScaleOffset] _Texture4 ("Texture 4", 2D) = "white" {}
		...
		_White ("White", Range(0.0 , 2.0)) = 0.25
	...
	SubShader {
		...
		Pass {
			...
			float4 _MainTex_ST;
			float4 _Tint;
			uniform float _Transition1, _Transition2, _Transition3, _White;
			...
			float4 MyFragmentProgram (float2 uv : TEXCOORD0, float2 uvSplat : TEXCOORD1) : SV_TARGET {
				...
				return 	tex2D(_Texture1, uv) * splat.r +
						tex2D(_Texture2, uv) * (_White - _Transition1);
...
//						tex2D(_Texture2, uv) * splat.g +
//						tex2D(_Texture3, uv) * splat.b +
//						tex2D(_Texture4, uv) * (_Transition1 - splat.r - splat.g - _White );
